...
function spawnPhantom(site, path, streaming, options, cb) {
  ...
  if (!streaming) {
    ...
    var stderrChunks = []
    phantomProc.stderr.on('data', function (chunk) {
      stderrChunks.push(chunk)
    })
    phantomProc.stderr.on('end', function (chunk) {
      var buf = stderrChunks.shift()
      if (!buf) return
      var str = buf.toString().trim()
      if (!str) return
      var err
      try {
        var parsedError = JSON.parse(str)
        err = new Error(parsedError.msg)
        err.isNon200Error = parsedError.isNon200Error
        err.isJsError = parsedError.isJsError
      } catch (JSONerr) {
        err = new Error(str)
        err.unknownError = true
        return handleError(err)
      }
      if (options.errorIfStatusIsNot200 && err.isNon200Error) {
        return handleError(err)
      }
      if (options.errorIfJSException && err.isJsError) {
        return handleError(err)
      }
    })
